...
"""
    multiscale(alg::MultiScaleAlgorithm, e::InformationMeasure, est::DifferentialInfoEstimator, x; kwargs...)
    multiscale(alg::MultiScaleAlgorithm, e::InformationMeasure, est::ProbabilitiesEstimator, x; kwargs...)
...
- `e::InformationMeasure`. A valid [entropy type](@ref entropies), i.e. `Shannon()` or `Renyi()`.
...
- `est`. For discrete entropy, `est` is a [`ProbabilitiesEstimator`](@ref), which determines
    ...
    continuous/differential entropy, `est` can be a [`DifferentialInfoEstimator`](@ref),
...
"""
    multiscale_normalized(alg::MultiScaleAlgorithm, e::InformationMeasure,
...
Note: this doesn't work if `est` is an [`DifferentialInfoEstimator`](@ref).
"""
function multiscale_normalized(alg::MultiScaleAlgorithm, e::InformationMeasure, est, x)
    msg = "`multiscale_normalized` not implemented for $e $(typeof(est)) on data type $(typeof(x))"
    throw(ArgumentError(msg))
end

multiscale_normalized(alg::MultiScaleAlgorithm, e::InformationMeasure, est::DifferentialInfoEstimator, x) =
