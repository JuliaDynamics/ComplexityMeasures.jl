...
Formally, when used with [`encode`](@ref), an ``m``-element state vector
...
function encode(encoding::AmplitudeEncoding, x::AbstractVector)
    ...
    return discretize_unit_interval(encoding, Λ_normalized)
end

function discretize_unit_interval(encoding::Encoding, Λ_normalized::Real)
    return ceil(Int, Λ_normalized / (1 / encoding.n))
end
