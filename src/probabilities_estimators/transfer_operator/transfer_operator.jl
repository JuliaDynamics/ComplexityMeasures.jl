...
A [`ProbabilitiesEstimator`](@ref) based on the transfer (Perron-Frobenius) operator.
This estimator only works with count based outcome spaces and
assumes that the input data are sequential (time-ordered).

When used with [`probabilities`](@ref), then the transfer operator
is approximated using a selected outcome space, by counting occurrences of transitions
between each outcome.
Probabilities are estimated as the invariant measure associated with that transfer operator.

See also [`transferoperator`](@ref).

## Inputs

`approximation_method` decides how the invariant measure should be calculated.
Two options exist: [`ApproximationIterative`](@ref), [`ApproximationEigen`](@ref).
...
`boundary_condition` decides the transition from the last observed outcome:

- `:circular` adds a transition to the first one,
- `:random` adds a randomly chosen transition from the already observed,
- `:none` adds nothing (default).

When constructing the transfer operator from time series using [`transferoperator`](@ref)
...
#N,tol,delta,rng
pars_default_iterative = (200, 1.0e-8, 1.0e-8, Random.default_rng())
...
#constructor with default method parameters
ApproximationEigen() = ApproximationEigen(
    KrylovDefaults.tol,
...
function transferoperator(
        o::OutcomeSpace, x::Array_or_SSSet;
        boundary_condition = :none,
        approximation_method = ApproximationIterative()
    )
...
function invariantmeasure(to::TransferOperatorApproximation{<:OutcomeSpace, ApproximationIterative})
    ...
    while counter <= N && distance >= tolerance
        ...
        if (
                check_pts_counter <= num_checkpts &&
...
function invariantmeasure(to::TransferOperatorApproximation{<:OutcomeSpace, ApproximationEigen})
    ...
    defaults = (
        tol = to.approximation_method.tol.x,
