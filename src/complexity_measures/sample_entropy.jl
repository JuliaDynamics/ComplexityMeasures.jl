...
"""
    sample_entropy(x::AbstractVector{T}; r::Real = 0.1, m::Int = 2,
        metric = Chebyshev(), normalize::Bool = false) where T <: Real

Estimate the sample entropy (Richman & Moorman, 2000)[^Richman2000] for the univariate
timeseries `x`, which is defined as

```math
SampEn(m, r) = \\lim_{N \\to \\infty} \\left[ -\\ln \\dfrac{A^{m+1}(r)}{B^m(r)} \\right],
```

where ``A^{m+1}(r)`` and ``B^{m}(r)`` are defined below.
...
If `Aᵐ⁺¹ == 0.0`, then `Inf` is returned.
...
"""
function sample_entropy(x::AbstractVector{T}; r::Real = 0.1, m::Int = 2,
        metric = Chebyshev(), normalize::Bool = false) where T <: Real
    ...
    if normalize
        # Richman & Moorman provide these constraints for the possible nonzero values
        # of the sample entropy. Use these values to scale to unit interval.
        N = length(x)
