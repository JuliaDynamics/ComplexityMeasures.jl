...
@testset "Dispersion" begin
    ...
    res = information(Renyi(base = MathConstants.e, q = 1), o, x)
    ...
    # their previous step.
    res_norm = information_normalized(Renyi(base = MathConstants.e, q = 1), o, x)
    @test round(res_norm, digits = 2) == 0.84

    @testset "outcomes" begin
        o = Dispersion(m = 3, c = 4)
        Ω = outcome_space(o)
        @test length(Ω) == total_outcomes(o) == 4^3
