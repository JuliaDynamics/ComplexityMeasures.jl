...
@test length(ps) == 3
@test sort(ps.p) == [0.25, 0.25, 0.5]
h = information(Renyi(base = 2), o, x)
@test h == 1.5
...
extent = (2, 2)
lag = (1, 1)
o = SpatialOrdinalPatterns((extent, lag), x; periodic = false)
@test information(Renyi(base = 2), o, x) == 1.5
...
ps, outs = probabilities_and_outcomes(o, x)
...
extent = (2, 2, 2)
lag = (1, 1, 1)
o2 = SpatialOrdinalPatterns((extent, lag), w; periodic = false)
@test information(Renyi(), o1, w) == information(Renyi(), o2, w)

# and to this stencil written as a matrix
stencil = [1; 1;; 1; 1;;; 1; 1;; 1; 1]
o3 = SpatialOrdinalPatterns(stencil, w; periodic = false)
@test information(Renyi(), o1, w) == information(Renyi(), o3, w)
