@testset "Histogram estimation" begin
    ...
    @test Entropies.fasthist(D) isa Vector{Int}
    @test Entropies.fasthist(D2) isa Vector{Int}
    @test sum(Entropies.fasthist(x)) == length(x)
    @test sum(Entropies.fasthist(D)) == length(D)
    @test sum(Entropies.fasthist(D2)) == length(D2)
end
