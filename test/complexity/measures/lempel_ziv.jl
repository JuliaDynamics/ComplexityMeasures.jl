...
@test complexity(LZ76(), x) == 7

# A very regular signal should have normalized LZ76-complexity close to zero.
ts = sin.(0:0.5:100000)
μ = mean(ts)
x = [x <= μ ? 0 : 1 for x in ts]
c = complexity_normalized(LZ76(), x)
@test 0 ≤ c ≤ 0.05

# A random sequence should have normalized LZ76-complexity close to 1
