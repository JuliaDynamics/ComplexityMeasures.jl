...
# The analytical results below are stated by Xue & Deng (2023) in nats, so we pin
# `base = ℯ`; at any other base both sides are scaled by the same 1/log(base) factor.

# Example 3.5 from Xue & Deng (2023) (equivalence of Tsallis entropy/information for
# two-element distributions)
x = [0.2, 0.4, 0.4]
h = information(Tsallis(; q = 2, base = ℯ), UniqueElements(), x)
j = information(TsallisExtropy(; q = 2, base = ℯ), UniqueElements(), x)
@test 2h ≈ 2j ≈ 8 / 9
...
# The entropy/extropy equivalence is independent of the base, since both are scaled
# by the same factor.
x = [0.2, 0.4]
@test information(Tsallis(; q = 2, base = 2), UniqueElements(), x) ≈
      information(TsallisExtropy(; q = 2, base = 2), UniqueElements(), x)
