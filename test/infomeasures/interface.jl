...
@testset "info interface: convenience methods" begin
    ...
    @test information(ML(Shannon()), probs) == information(Shannon(), probs)
    ...
    @test information_maximum(ML(s), e2) == information_maximum(s, e2)
    @test information_normalized(ML(s), e1, x) == information_normalized(s, e1, x)
end
...
@testset "info interface: errors" begin
    ...
    # some new measure
    struct SomeNewMeasure <: InformationMeasureDefinition end
...
@testset "info interface: normalization" begin
    ...
    @test information_maximum(ML(s), e2) == 1
    ...
    @test information_normalized(ML(s), e1, x) > 0
