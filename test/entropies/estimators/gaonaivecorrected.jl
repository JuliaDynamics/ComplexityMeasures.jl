...
npts = 1000000
ea = entropy(Shannon(), GaoNaiveCorrected(k = 5), rand(npts))
ea_n = entropy(Shannon(; base = ℯ), GaoNaiveCorrected(k = 5), randn(npts))
ea_n3 = entropy(Shannon(; base = 3), GaoNaiveCorrected(k = 5), randn(npts))
...
x = rand(1000)
@test_throws ArgumentError entropy(Renyi(q = 2), GaoNaiveCorrected(), x)

# Default is Shannon base-2 differential entropy
est = GaoNaiveCorrected()
